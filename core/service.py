...
from core.models import Game, UserRole, Coach, TeamStat, Player, PlayerStat, Team, UserLoginDetails
...
class BasketBallService:
    ...
    def get_coach_details(user_id):
        try:
            ...
            return JsonResponse(
                {'success': True, 'team_name': team_name, 'average_score': average_score,
                 'player_data': new_player_array})
        except Coach.DoesNotExist:
            return JsonResponse({'success': False, "error": "user not found"}, status=status.HTTP_404_NOT_FOUND)

    @staticmethod
    def get_player_score(request, user_id):
        try:
            user_role = UserRole.objects.get(user_id=user_id)
            if user_role.role.type == 'P':
                return JsonResponse({'success': False, "error": "not Authorized to access"},
            ...
            return JsonResponse(
                {'success': True, 'team_name': team_name, 'average_score': av_score_obj,
                 'player_data': new_player_array})
        except Coach.DoesNotExist:
            return JsonResponse({'success': False, "error": "user not found"}, status=status.HTTP_404_NOT_FOUND)

    @staticmethod
    def get_all_team_details(user_id):
        try:
            user_role = UserRole.objects.get(user_id=user_id)
    ...
    @staticmethod
    def view_login_statistics(user_id):
        try:
            user_role = UserRole.objects.get(user_id=user_id)
            if user_role.role.type != 'A':
                return JsonResponse({'success': False, "error": "not Authorized to access"},
